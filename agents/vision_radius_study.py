...
def summarize(values):
    ...
    return float(arr.mean()), float(arr.std(ddof=0)), int(arr.max()), int(arr.min())
...
def main():
    ...
    parser.add_argument("--mdp-qstar", type=str, default="mdp_qstar.npy")
    parser.add_argument("--nn-model", type=str, default="q_network.pt")
    parser.add_argument("--lstm-model", type=str, default="agents/dqn_lstm_policy.pt")
    ...
    agents = {}
    if os.path.exists(args.nn_model):
        agents["run_nn_agent"] = load_feedforward_agent(args.nn_model, device)
    else:
        print(f"Skipping run_nn_agent: model {args.nn_model} not found.")

    if os.path.exists(args.lstm_model):
        lstm_hidden_sizes = parse_lstm_sizes(args.lstm_hidden_sizes)
        lstm_net = LSTMNetwork(
            ...
            args.lstm_hidden,
            args.lstm_layers,
            args.lstm_mlp_dropout,
            args.lstm_dropout,
        ).to(device)
        try:
            lstm_net.load_state_dict(torch.load(args.lstm_model, map_location=device))
            lstm_net.eval()
            agents["run_lstm_agent"] = LSTMRunner(lstm_net, device)
        except RuntimeError as exc:
            print(f"Skipping run_lstm_agent: {exc}")
    else:
        print(f"Skipping run_lstm_agent: model {args.lstm_model} not found.")

    if not os.path.exists(args.mdp_qstar):
        raise FileNotFoundError(f"MDP q* file not found: {args.mdp_qstar}")
    qstar = np.load(args.mdp_qstar)
    state_space_cache = {}
        ...
        for radius in radii:
            for run_idx in range(args.runs_per_radius):
                ...
                if agent_name == "run_mdp_agent":
                    steps = run_mdp_episode(radius, args.max_steps, args.render, qstar, state_space_cache)
                elif agent_name == "run_lstm_agent":
                    steps = agents[agent_name].run_episode(radius, args.max_steps, args.render)
                else:
    ...
    for agent_name in eval_agents:
        ...
        for radius in radii:
            ...
            if not values:
                ...
                continue
            mean_val, std_val, longest, shortest = summarize(values)
            print(
                f"  Vision {radius:3d} -> mean {mean_val:.2f} steps | std {std_val:.2f} | longest {longest} | shortest {shortest}"
            )
